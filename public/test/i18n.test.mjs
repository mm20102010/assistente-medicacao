...
app += '\nglobalThis.__parseDateTimeText=parseDateTimeText;globalThis.__parseTextHistory=parseTextHistory;globalThis.__toCsv=toCsv;globalThis.__canonicalRelief=canonicalRelief;globalThis.__validateState=validateState;globalThis.__watchMedicationEventToRecord=watchMedicationEventToRecord;';
...
const appVersion=index.match(/manifest\.webmanifest\?v=([0-9.]+)/)?.[1];assert.ok(appVersion,'versão do app deve estar presente no index');assert.match(index,/mm-registro-i18n\.js\?v=2\.47/);assert.ok(index.includes(`src="i18n.js?v=${appVersion}"`));assert.match(index,/id="languageBtn"/);assert.equal((index.match(/data-locale-mode=/g)||[]).length,4);
const sw=fs.readFileSync(new URL('../public/sw.js',import.meta.url),'utf8');assert.match(sw,/mm-registro-i18n\.js\?v=2\.47/);assert.ok(sw.includes(`"./i18n.js?v=${appVersion}"`));
